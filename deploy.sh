...
function backup_database {
    ...
    docker-compose exec -T db pg_dump -Fc -U di_website -d di_website -f $current_date.backup
    docker cp diwebsiteredesign_db_1:$current_date.backup $DATABASE_BACKUP
